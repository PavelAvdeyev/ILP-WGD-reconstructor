...
def create_ilp_formulation_for_restricted_median(cfg):
    try:
        ...
        print((len(cfg.ind_median_set) // 2 + cfg.number_of_cycles + cfg.number_of_even_paths // 2))
        logger.info("CREATING BIG CONSTRAINT")
        model.addConstr(tilde_bs[0].sum('*') + tilde_bs[1].sum('*') - dot_rs.sum('*') -
                        hat_bs[0].sum('*') - hat_bs[1].sum('*') == len(cfg.ind_median_set) // 2 +
                        cfg.number_of_cycles + cfg.number_of_even_paths // 2)

        logger.info("CREATING OBJECTIVE FUNCTION.")
        model.setObjective(tilde_bs[2].sum('*') - 0.5 * dot_rs.sum('*') - hat_bs[2].sum('*'),
...
def get_param_of_solution_for_restricted_median(model, cfg, rs, dot_rs):
    ...
    number_of_vertices = len(cfg.ind_median_set.union(cfg.ind_vertex_sets[2]))
    ...
    block_order = get_genome_graph_from_vars(rs=rs, r_dot=dot_rs,
                                             gene_set=cfg.ind_median_set,
                                             telomer_set=cfg.allowable_telomers,
                                             edge_set=cfg.allowable_median_edges,
                                             ind2vertex=cfg.ind2vertex)

...
def do_test_job(input_directory, gurobi_log_file):
    logging.info('Let\'s do comparison test for the GGHP-ILP. Input directory is {0}'.format(input_directory))
...
def test_run():
    parser = argparse.ArgumentParser(description="ILP solver for the Guided Genome Halving Problem (GGHP)",
    ...
    args.log_file = os.path.join(args.paper_test, "gghp.log")
    args.gurobi_log_file = os.path.join(args.paper_test, "gurobi_gghp.log")
...
def main():
    ...
    args.log_file = os.path.join(args.out_dir, "gghp.log")
    args.gurobi_log_file = os.path.join(args.out_dir, "gurobi_gghp.log")
    enable_logging(args.log_file, overwrite=False)

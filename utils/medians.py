...
from utils.common import remove_singletons_wrt_gene_set
...
class MedianConf(object):
    def __init__(self, genomes, name, log_file, tl):
        ...
        self.ind_ancestral_set = set(reduce(operator.or_,
                                            [self.ind_cbg_p_i_vertex_sets[i] & self.ind_cbg_p_i_vertex_sets[j] for i, j
                                             in
...
def remove_known_singletons(genomes):
    ...
    for i in range(len(genomes)):
        ...
        genomes[i] = genome
        total_number += numb_of_singletons

    return genomes, total_number
